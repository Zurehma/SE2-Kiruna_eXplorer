...
const setLink = async (linkData) => {
  try {
    const response = await fetch(`${VITE_API_URL}/documents/link`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({
...
const logIn = async (credentials) => {
  try {
    const response = await fetch(VITE_API_URL + "/sessions/login", {
      method: "POST",
      headers: {
 ...
 */
const logOut = async () => {
  return await fetch(VITE_API_URL + "/sessions/logout", {
    method: "DELETE",
    credentials: "include",
  }).then(handleInvalidResponse);
};
const getUserInfo = async () => {
  return await fetch(VITE_API_URL + "/sessions/current", {
    credentials: "include",
  })
    .then(handleInvalidResponse)
    .then((response) => response.json());
